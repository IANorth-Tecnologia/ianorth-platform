...
def get_dashboard_data(db: Session = Depends(get_db)):
    ...
    query_totais = text("""
        SELECT 
            camera_id, 
            ISNULL(SUM(final_count), 0) as total
        FROM lotes
        WHERE CAST(start_time AS DATE) = CAST(GETDATE() AS DATE)
        GROUP BY camera_id
    """)
    ...
    totais_formatados = []
    cores = ['#4F46E5', '#10B981', '#F59E0B', '#EF4444'] # Cores para as barras
    ...
    for row in res_hora:
        ...
        dados_hora_dict[hora_str][cam_id] = row.total
    
    dados_producao_hora = list(dados_hora_dict.values())

    return {
        "kpis": {
            "pecas_hoje": res_kpis.total_pecas,
            "lotes_hoje": res_kpis.total_lotes,
            "maquinas_ativas": res_kpis.maquinas_ativas
        },
        "totais_por_maquina": totais_formatados,
        "producao_hora": dados_producao_hora
    }